...
def is_for_me(event):
    ...
    type = event.get('type')
    if type and type == 'message' and not(event.get('user')==JOHNSONCB1_SLACK_ID):
        ...
        if johnsoncb1_slack_mention in text.strip().split():
            return True
...
def handle_message(message, user, channel):
    if is_hi(message):
        user_mention = get_mention(user)  # format the user's name 
        post_message(message=say_hi(user_mention), channel=channel)  # post the message and then @ user
    elif is_bye(message):
